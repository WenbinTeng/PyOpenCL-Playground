...
int main() {
  ...
  cl_int status;
  // Search the number of platforms
  cl_uint numPlatforms = 0;
  status = clGetPlatformIDs(0, NULL, &numPlatforms);
  // Allocate enough space for each platform object
  cl_platform_id *platforms = NULL;
  platforms = (cl_platform_id *)malloc(numPlatforms * sizeof(cl_platform_id));
  // Fill the allocated space with platform objects
  status = clGetPlatformIDs(numPlatforms, platforms, NULL);
  // Search the number of devices
  cl_uint numDevices = 0;
  status = clGetDeviceIDs(platforms[0], CL_DEVICE_TYPE_ALL, 0, NULL, &numDevices);
  // Allocate enough space for each device object
  cl_device_id *devices;
  devices = (cl_device_id *)malloc(numDevices * sizeof(cl_device_id));
  // Fill the allocated space with device objects
